...
import { ArrowLeft, Calendar, Upload, Trash2, Plus, MapPin, Check, AlertCircle, RefreshCw, DollarSign, Users, Info } from "lucide-react";
...
export default function EditUpcomingTourPage() {
    ...
    async function loadData() {
      try {
        ...
        if (data) {
          ...
          setBookedSlots(data.bookedSlots || 0);
          ...
          setError("Upcoming Tour not found");
        }
      } catch (err) {
        console.error("Fetch error:", err);
        setError("Failed to load upcoming tour details");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/upcoming-tours/update", {
        method: "PUT",
      ...
      if (data.success) {
        ...
        setError(data.error || "Failed to update upcoming tour");
      }
    } catch (err) {
      console.error(err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
        ...
        <p className="text-charcoal/50 font-semibold">Loading event details...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-primary flex items-center gap-2 font-serif">
            ...
            Edit Upcoming Tour / Event
          </h1>
          <p className="text-charcoal/70 mt-1 font-medium">Modify existing scheduled details, dates, and slots.</p>
        </div>
      </div>
      ...
      {success && (
        <div className="flex items-center gap-3 bg-primary/10 border border-primary/20 text-primary p-4 rounded-xl font-semibold">
          ...
          <span>Tour Event Updated Successfully! Redirecting...</span>
        </div>
      )}
          ...
          <div className="bg-white border border-primary/10 shadow-sm rounded-2xl p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-charcoal/50 flex items-start gap-1 font-medium leading-relaxed">
                ...
                Guides are loaded from Sanity CMS authors.
              </p>
            </div>
          </div>
